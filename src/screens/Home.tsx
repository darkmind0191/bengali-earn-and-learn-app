...
useEffect(() => {
  ...
  async function loadDailyBonus(): Promise<number> {
    if (!profile) return 0;
    ...
    const isCheckedInToday = !!todayData;
 ...
 async function handleCheckIn() {
   ...
   if (error) {
     if (
       error.message.includes('daily_bonus_unique_user_date') ||
       error.message.includes('duplicate key')
     ) {
       alert('আজকের চেক-ইন ইতিমধ্যে সম্পন্ন হয়েছে');
...
await supabase.rpc('increment_balance', {
  ...
  p_amount: 10,
   ...
   alert('আজকের ১০ পয়েন্ট বোনাস যোগ হয়েছে!');

   setCheckedInToday(true);

   const newStreak = await loadDailyBonus();

   if (newStreak >= 3) {
     await supabase.rpc('unlock_badge', {
       p_user_id: userId,
       p_key: 'streak_3',
       p_name: '3 Day Streak',
       p_icon: '🔥',
        ...
        <div className="rounded-3xl bg-gradient-to-br from-brand-500 via-brand-600 to-blue-600 p-6 text-white shadow-xl shadow-brand-500/25 relative overflow-hidden">
          ...
          <div className="relative">
            ...
            <div className="flex items-end gap-2">
              <span className="text-4xl font-bold">{profile?.balance ?? 0}</span>
